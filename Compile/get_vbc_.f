...
!$AGRIF_END_DO_NOT_TREAT
     ...
     &                                                            ))
        ...
        do j=Jstr,Jend
          do i=Istr,Iend
            cff  = max(1.D0, 30.D0/(h(i,j)+h(i-1,j)))
            bustr(i,j)=cff*rdrg*u(i,j,1,nrhs)
          enddo
        enddo
        do j=Jstr,Jend
          do i=Istr,Iend
            cff  = max(1.D0, 30.D0/(h(i,j)+h(i,j-1)))
            bvstr(i,j)=cff*rdrg*v(i,j,1,nrhs)
          enddo
        enddo
      endif
      cff=0.75D0/dt
     ...
     &                   ABS(v(i,j,1,nrhs))*cff1)
        enddo
      enddo
      call exchange_u2d_tile (Istr,Iend,Jstr,Jend,bustr(-2,-2))
      call exchange_v2d_tile (Istr,Iend,Jstr,Jend,bvstr(-2,-2))
      call exchange_u2d_tile (Istr,Iend,Jstr,Jend,sustr(-2,-2))
      call exchange_v2d_tile (Istr,Iend,Jstr,Jend,svstr(-2,-2))
      call exchange_r2d_tile (Istr,Iend,Jstr,Jend,
     &                                      stflx(-2,-2,itemp))
      call exchange_r2d_tile (Istr,Iend,Jstr,Jend,srflx(-2,-2))
      call exchange_r2d_tile (Istr,Iend,Jstr,Jend,
     &                                      stflx(-2,-2,isalt))
      return
      end
