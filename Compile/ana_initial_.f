...
C$    integer*4 omp_get_thread_num
     ...
     &           year2day=365.25D0, day2year=1.D0/365.25D0)
      ...
      real u0,v0, randomn, rr, xcenter, ycenter
      parameter (u0=0.0D0)
      parameter (v0=0.0D0)
      ...
      if (nrrec.eq.0) then
        do j=JstrR,JendR
          do i=IstrR,IendR
            zeta(i,j,1)=-f(i,j)/g*((yr(i,j)-el/2.D0)*u0
     &                            -(xr(i,j)-xl/2.D0)*v0)
      ...
      if (nrrec.eq.0) then
        xcenter=xl*(float(LLm)+1)/(2*float(LLm))
        ycenter=el*(float(MMm)+1)/(2*float(MMm))
        do k=1,N
          do j=JstrR,JendR
            do i=IstrR,IendR
              rr = sqrt((xr(i,j)-xcenter)**2 +(yr(i,j)-ycenter)**2)
              call random_number(randomn)
              t(i,j,k,1,itemp)=15.0D0
     &                        -0.002D0*TANH((z_r(i,j,k)
     &                                  + h(1,1)/2
     &                                  + 
     &                                  250.0D0*cos((i+iminmpi-1)*pi/96)
     &                                             )*pi*1.0D-2)
              t(i,j,k,2,itemp)=t(i,j,k,1,itemp)
              t(i,j,k,1,isalt)=35.4D0
     &                        -0.1D0*TANH((z_r(i,j,k)
     &                                  + h(1,1)/2
     &                                  + 
     &                                  250.0D0*cos((i+iminmpi-1)*pi/96)
     &                                             )*pi*1.0D-2)
